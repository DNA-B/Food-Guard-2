...
public class GroupController {
        ...
        @Operation(summary = "그룹 내 음식 목록 조회", description = "그룹에 속한 음식 목록을 조회합니다.")
        @ApiResponses(value = {
        ...
        public ResponseEntity<List<FoodResponse>> getFood(
                ...
                return ResponseEntity.ok(groupService.findAllFoodById(id));
        }
        ...
        @PostMapping("/{id}/exit")
        public ResponseEntity<Void> groupExit(
                        @Parameter(description = "그룹 ID", example = "1") @PathVariable("id") Long id,
                        @Parameter(hidden = true) @AuthenticationPrincipal Long userId) {
