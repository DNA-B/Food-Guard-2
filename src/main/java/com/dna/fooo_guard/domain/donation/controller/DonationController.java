...
public class DonationController {
        ...
        @ApiResponses(value = {
                        ...
                        @ApiResponse(responseCode = "404", description = "DONATION_NOT_FOUND : 해당 나눔글을 찾을 수 없음", content = @Content)
        })
        @GetMapping("/{id}")
        ...
        @Operation(summary = "나눔 완료 처리", description = "나눔 작성자가 나눔 상태를 완료로 변경합니다.")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "완료 처리 성공 (OK)"),
                        @ApiResponse(responseCode = "403", description = "ACCESS_DENIED : 접근 권한이 없습니다 (작성자가 아님)", content = @Content),
                        @ApiResponse(responseCode = "404", description = "DONATION_NOT_FOUND : 해당 나눔글을 찾을 수 없음", content = @Content)
        })
        @PatchMapping("/{donationId}/complete")
        public ResponseEntity<Void> completeDonation(
                        @Parameter(description = "나눔 ID", example = "1") @PathVariable Long donationId,
                        @Parameter(hidden = true) @AuthenticationPrincipal Long userId) {
